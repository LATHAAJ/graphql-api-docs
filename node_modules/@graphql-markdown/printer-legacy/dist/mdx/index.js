...
exports.mdxModule = exports.mdxDeclaration = void 0;
 ...
 */
const formatMDXBadge = (badge) => {
    return `<mark class="gqlmd-mdx-badge">${badge.text}</mark>`;
};
/**
 ...
 */
const formatMDXAdmonition = ({ text, title, type, icon }, _meta) => {
    return `${strings_1.MARKDOWN_EOP}<fieldset class="gqlmd-mdx-admonition-fieldset">${strings_1.MARKDOWN_EOL}<legend class="gqlmd-mdx-admonition-legend"><span class="gqlmd-mdx-admonition-legend-type gqlmd-mdx-admonition-legend-type-${type.toLocaleLowerCase()}">${icon ?? type.toUpperCase()} ${title}</span></legend>${strings_1.MARKDOWN_EOL}<span>${text}</span>${strings_1.MARKDOWN_EOL}</fieldset>`;
};
/**
...
const formatMDXBullet = (text = "") => {
    return `<span class="gqlmd-mdx-bullet">&nbsp;●&nbsp;</span>${text}`;
};
/**
...
const formatMDXDetails = ({ dataOpen }) => {
    return `${strings_1.MARKDOWN_EOP}<details class="gqlmd-mdx-details">${strings_1.MARKDOWN_EOL}<summary class="gqlmd-mdx-details-summary"><span className="gqlmd-mdx-details-summary-open">${dataOpen.toUpperCase()}</span></summary>${strings_1.MARKDOWN_EOL}</details>${strings_1.MARKDOWN_EOP}`;
};
/**
...
const formatMDXSpecifiedByLink = (url) => {
    return `<span class="gqlmd-mdx-specifiedby">Specification<a class="gqlmd-mdx-specifiedby-link" target="_blank" href="${url}" title="Specified by ${url}">⎘</a></span>`;
};
/**
...
const formatMDXNameEntity = (name, parentType) => {
    const parentName = parentType
        ...
        : "";
    return `<span class="gqlmd-mdx-entity">${parentName}<code class="gqlmd-mdx-entity-name">${name}</code></span>`;
};
/**
...
const formatMDXFrontmatter = (_props, formatted) => {
    return formatted
        ? [strings_1.FRONT_MATTER_DELIMITER, ...formatted, strings_1.FRONT_MATTER_DELIMITER].join(strings_1.MARKDOWN_EOL)
        : "";
};
...
const defaultModule = {
    ...
    mdxDeclaration: exports.mdxDeclaration,
};
const mdxModule = async (mdxPackage) => {
    if (!mdxPackage) {
        return defaultModule;
    }
    const module = (mdxPackage.default ?? mdxPackage);
    return {
        ...
        formatMDXSpecifiedByLink: module.formatMDXSpecifiedByLink ?? formatMDXSpecifiedByLink,
        mdxDeclaration: module.mdxDeclaration ?? exports.mdxDeclaration,
    };
};
exports.mdxModule = mdxModule;
